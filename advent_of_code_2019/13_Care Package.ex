defmodule Day13 do
  ...
  defp write_output(context, modes) do
    ...
    if length(new_output) >= 3 do
      [x, y, tile_id] = new_output
      # context = draw_color(context, draw_color) |> turn(direction) |> move()
      context = draw_tile(context, [x, y, tile_id])
  ...
  defp draw_tile(%{board: board_context} = context, [x, y, tile_id] = _list) do
    new_map_info =
      case Enum.count(board_context.map_info, fn e -> e.x == x and e.y == y end) do
        0 -> [%{x: x, y: y, type: tile_id} | board_context.map_info]
        _ -> board_context.map_info
      end

    new_board_context = %{board_context | map_info: new_map_info}
  ...
  defp part_1(memory) do
    ...
    Enum.count(result.board.map_info, fn e -> e.type == 2 end) |> IO.inspect()
  end
...
File.read!("inputs/13.txt") |> Day13.run()
